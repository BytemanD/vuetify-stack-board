      ...
      <v-col>
        ...
        <v-menu offset-y>
          <template v-slot:activator="{ on, attrs }">
            <v-btn small color="info" v-bind="attrs" v-on="on" :disabled="table.selected.length == 0"><v-icon
        ...
        <v-btn small color="primary" @click="migrateDialog.open()" :disabled="table.selected.length == 0">迁移</v-btn>
        <v-btn small color="warning" @click="evacuateDialog.open()" :disabled="table.selected.length == 0">疏散</v-btn>
        <v-btn small color="warning" @click="table.openResetStateDialog()"
        ...
        <v-btn small color="info" @click="computing.serverTopology.open(table)">拓扑</v-btn>
      </v-col>
      <v-col cols='2'>
          <v-switch hide-details color="warning" v-model="table.deleted" class="my-auto" label="已删除" v-on:click="table.refresh()">已删除</v-switch>
      </v-col>
      <v-col cols="2">
          <v-text-field small dense hide-details v-model="table.search" append-icon="mdi-magnify" label="搜索" single-line></v-text-field>
      </v-col>
