  ...
  <v-dialog v-model="display" width="1000" scrollable>
    <v-card>
      ...
      <v-card-text class="pa-4">
        <v-btn x-small fab color="primary" class="mr-1" @click="openNewSGRuleDialog()"><v-icon>mdi-plus</v-icon></v-btn>
        <v-btn small color="error" @click="dialog.deleteSelected()" :disabled="dialog.selected.length == 0">删除</v-btn>

        <v-data-table :headers="dialog.headers" :items="dialog.securityGroup.security_group_rules"
          :items-per-page="dialog.itemsPerPage" :search="dialog.search" dense show-select v-model="dialog.selected"
