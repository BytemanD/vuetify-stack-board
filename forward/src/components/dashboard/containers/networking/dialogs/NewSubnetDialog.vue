        ...
        <v-card>
            ...
            <v-card-text>
                <v-row>
                    ...
                    <v-col cols="6">
                        <v-text-field dense outlined  label="CIDR" placeholder="请输入cidr" v-model="dialog.cidr" :error="!dialog.cidr" :rules="[dialog.checkCidr]"></v-text-field>
                        <v-select dense outlined :items="dialog.ipVersions" label="IP版本" v-model="dialog.ipVersion"></v-select>
                    </v-col>
                    <v-col cols="6">
                        <v-switch dense v-model="dialog.enableDhcp" label="启用DHCP" class="my-auto"></v-switch>
                    </v-col>
                </v-row>
                <v-alert dense outlined type="error" v-if="dialog.errorMessage">[[ dialog.errorMessage ]]</v-alert>
            </v-card-text>
            <v-divider></v-divider>
            <v-card-actions>
                <v-spacer></v-spacer>
