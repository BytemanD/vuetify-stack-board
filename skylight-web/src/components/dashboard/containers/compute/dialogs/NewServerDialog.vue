        ...
        <v-card>
            <v-card-text height="300" class="pa-0">
                <v-stepper elevation="0" vertical height="510">
                    <v-stepper-step step="1" color="success" editable="editable">
                        <h2>基本设置</h2>
                    </v-stepper-step>
                    <v-stepper-content step="1">
                        <v-row>
                            ...
                            <v-col cols="6">
                                <v-select :items="dialog.flavors" label="规格" density='compact' item-text="name"
                                    item-value="id" v-model="dialog.params.flavor" :error="!dialog.params.flavor">
                                </v-select>
                                <v-select :items="dialog.images" label="镜像" density='compact' item-text="name"
                                    item-value="id" v-model="dialog.params.image" :error="!dialog.params.image">
                                </v-select>
                            </v-col>
                            <v-col cols="6">
                                <v-select :items="dialog.networks" clearable label="网络" density='compact' item-text="name"
                                    item-value="id" v-model="dialog.params.netId" @click="dialog.refresNetworks()">
                                </v-select>
                                <v-select clearable density='compact' :items="dialog.ports" label="端口" item-text="name"
                                    item-value="id" messages="如果选择端口，只能创建一台虚拟机。" v-model="dialog.portId"
                                    @click="dialog.refresPorts()">
                                </v-select>
                            </v-col>
                            <v-col cols="2">
                                <v-switch v-model="dialog.useBdm" label="创建卷" class="my-auto" hide-details></v-switch>
                            </v-col>
                            <v-col cols="4">
                                <v-select hide-details :disabled="!dialog.useBdm" :items="dialog.volumeTypes" clearable
                                    label="卷类型" density='compact' item-text="name" item-value="id"
                                    @click="dialog.refreshVolumeTypes()" v-model="dialog.volumeType">
                                    <template v-slot:selection="{ item }"> {{ item.name }} </template>
                                </v-select>
                            </v-col>
                            <v-col cols="12">
                                <v-slider :disabled="!dialog.useBdm" hide-details v-model="dialog.volumeSize" ticks="always"
                                    label="系统卷大小" max="100" :min="dialog.volumeSizeMin" step="10">
                    ...
                    <v-stepper-step step="2" color="success" editable="editable">
                        <h2>{{ $t('security') }}</h2>
                    </v-stepper-step>
                    ...
                    <v-stepper-step step="3" color="success" editable="editable">
                        <h2>自定义</h2>
                    </v-stepper-step>
                    <v-stepper-content step="3">
                        <v-slider v-model="dialog.params.nums" label="实例数量" ticks="always" max="10" min="1"
                            :disabled="dialog.portId != null">
